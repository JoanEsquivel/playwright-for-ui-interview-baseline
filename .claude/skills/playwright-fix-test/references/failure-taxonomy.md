...
| **flaky** | intermittent; passes on retry; no other test involved | `--repeat-each=10`; trace of a failing attempt | Remove the race: wait for the specific post-condition, isolate data per test, avoid shared users, disable animations if needed |
